...
export const ForecastChart: React.FC<ForecastChartProps> = ({
  ...
  const [chartType, setChartType] = React.useState<
    "wealth" | "income" | "breakdown" | "stacked"
  >("wealth");
  ...
  const renderWealthChart = () => (
    <ResponsiveContainer width="100%" height={400}>
      <AreaChart data={chartData}>
        ...
        <XAxis dataKey="yearLabel" stroke="#64748b" fontSize={12} />
        ...
        <Card className="bg-gradient-to-br from-green-50 to-green-100 border-green-200">
          <CardContent className="p-4">
            ...
            <div className="text-xl font-bold text-green-800">
              {formatCurrency(
                projections[projections.length - 1]?.cumulativeWealth || 0,
              )}
            </div>
        ...
        <Card className="bg-gradient-to-br from-blue-50 to-blue-100 border-blue-200">
          <CardContent className="p-4">
            ...
            <div className="text-xl font-bold text-blue-800">
              {formatCurrency(
                projections.reduce((sum, p) => sum + p.savings, 0) /
                  projections.length,
              )}
            </div>
            <div className="text-xs text-blue-600">Per year average</div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-gradient-to-br from-purple-50 to-purple-100 border-purple-200">
          <CardContent className="p-4">
            ...
            <div className="text-xl font-bold text-purple-800">
              {formatCurrency(
                (projections[projections.length - 1]?.cumulativeWealth || 0) -
                  (projections[0]?.cumulativeWealth || 0),
              )}
            </div>
            <div className="text-xs text-purple-600">Net increase</div>
          </CardContent>
        </Card>
