...
export const GoalManager: React.FC<GoalManagerProps> = ({ netWorth, savingsRate, totalDebt }) => {
  ...
  const addGoal = () => {
    if (newGoal.name && newGoal.targetValue > 0) {
      ...
      setGoals(goalChecker.getAllGoals());
      setNewGoal({ name: '', type: 'netWorth', targetValue: 0, emailNotification: false });
  ...
  const getCurrentValue = (type: GoalType) => {
    switch (type) {
      ...
      case 'debtBalance': return totalDebt;
      default: return 0;
    }
  };
  ...
  const getProgress = (goal: Goal) => {
    if (goal.type === 'debtBalance') {
      ...
      const progress = goal.targetValue > 0 ? Math.max(0, 100 - (goal.currentValue / goal.targetValue) * 100) : 0;
  ...
  const getTypeLabel = (type: GoalType) => {
    switch (type) {
      ...
      case 'debtBalance': return 'Debt Balance';
      default: return type;
    }
  };
        ...
        <CardContent>
            ...
            <div className="space-y-4 p-4 border rounded-lg mb-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Select value={newGoal.type} onValueChange={(value) => setNewGoal({ ...newGoal, type: value as GoalType })}>
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="debtBalance">Debt Balance (Target Lower)</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                <div>
                  <Label htmlFor="targetValue">
                    Target Value {newGoal.type === 'savingsRate' ? '(%)' : '($)'}
                  </Label>
                  <Input
                    id="targetValue"
                    type="number"
                ...
                <div className="flex items-center space-x-2">
                  <Switch
                    ...
                    onCheckedChange={(checked) => setNewGoal({ ...newGoal, emailNotification: checked })}
                  />
          ...
          <div className="space-y-4">
            {goals.length === 0 ? (
              ...
              goals.map((goal) => (
                <Card key={goal.id} className={`${goal.achieved ? 'border-green-200 bg-green-50' : 'border-slate-200'}`}>
                  <CardContent className="p-4">
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-slate-600">{getTypeLabel(goal.type)}</p>
                        </div>
                      </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex justify-between text-sm text-slate-600">
                        <span>Current: {formatValue(goal.currentValue, goal.type)}</span>
                        <span>Target: {formatValue(goal.targetValue, goal.type)}</span>
                      </div>
                    </div>
                  </CardContent>
                </Card>
