  ...
  return (
    <div className="space-y-2 mt-4">
      <div>
        ...
        <div className="bg-slate-50 border rounded p-2 mt-1 text-gray-800">{logicText || <span className="text-gray-400">No logic</span>}</div>
      </div>
      <div>
        <span className="font-semibold text-gray-700">Logic (JSON):</span>
        <pre className="bg-slate-50 border rounded p-2 mt-1 text-xs overflow-x-auto">{JSON.stringify(logicJSON, null, 2)}</pre>
      </div>
    </div>
  );
};
