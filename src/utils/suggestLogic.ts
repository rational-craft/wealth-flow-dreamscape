...
export function suggestOperators(
  ...
  return OPERATORS[fieldType].filter((op) =>
    op.toLowerCase().includes(input.toLowerCase()),
  );
}
...
export function parseCondition(tokens: string[]): {
  ...
  const matchedOp = possibleOps.find(
    (op) => op.toLowerCase() === tokens[1].toLowerCase(),
  );
  if (!matchedOp)
    return { error: `Invalid operator "${tokens[1]}" for ${field.label}` };
