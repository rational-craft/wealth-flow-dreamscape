...
self.onmessage = function(e: MessageEvent<MonteCarloParams>) {
  const { initialWealth, annualContribution, expectedReturn, volatility, years, simulations } = e.data;
  ...
  const successRate = finalValues.filter(v => v > initialWealth * 2).length / simulations;
